    ...
    useEffect(() => {
        if (isMobile) {
            intervalRef.current = setInterval(() => {
                setActiveCard((prev) => (prev + 1) % journeyPhases.length);
        ...
        <section id="roadmap" className="py-32 text-theme-text overflow-hidden relative z-30 transition-colors duration-1000" style={{ background: `linear-gradient(to bottom, var(--theme-bg) 0%, #e7e5e4 100%)` }}>
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 relative">
                ...
                <div className="lg:hidden w-full">
                    {/* Main Carousel Card */}
                    <div className="relative w-full overflow-hidden rounded-[7px] border border-theme-text/10 bg-theme-surface shadow-lg mb-6" style={{ minHeight: '500px' }}>
                        <div className="relative w-full overflow-hidden" style={{ minHeight: '500px' }}>
                            <div 
                                className="flex w-full transition-transform duration-900 ease-[cubic-bezier(0.25,1,0.3,1)]"
                                style={{ 
                                    width: `${journeyPhases.length * 100}%`,
                                    transform: `translateX(-${activeCard * (100 / journeyPhases.length)}%)`
                                }}
                            >
                                {journeyPhases.map((phase, index) => (
                                    <div 
                                        ...
                                        style={{ 
                                            width: `${100 / journeyPhases.length}%`,
                                            minHeight: '500px' 
                                        }}
                                    >
                                        <img
                                        ...
                                        <div className="absolute inset-0 flex items-end">
                                            <div className="w-full bg-black/40 p-8">
                                                <h3 className="text-3xl font-serif font-bold text-white mb-3 text-left">
                                                    {phase.title}
                                                </h3>
                    ...
                    <div className="flex justify-center items-center gap-3 mb-6">
                        {journeyPhases.map((phase, index) => (
                            <button
                                ...
                                className={`rounded-full transition-all duration-300 ease-[cubic-bezier(0.25,1,0.3,1)] ${
                                    activeCard === index 
                                        ? 'w-8 h-2 bg-theme-accent' 
                                        : 'w-2 h-2 bg-theme-text/30 hover:bg-theme-text/50'
                                }`}
                ...
                <div className="hidden lg:grid grid-cols-12 gap-8 lg:gap-12 items-stretch">
                        ...
                        <div className="flex-1 relative overflow-hidden rounded-[7px] border border-theme-text/10 bg-theme-surface shadow-lg min-h-[500px]">
                            <div className="relative w-full h-full overflow-hidden">
                                <img
                                    ...
                                    className="absolute inset-0 w-full h-full object-cover transition-opacity duration-900 ease-[cubic-bezier(0.25,1,0.3,1)]"
                                />
                                {/* Dark overlay for text readability */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
                    ...
                    <div className="lg:col-span-6 grid grid-cols-4 gap-4 h-full items-start">
                        {journeyPhases.map((phase, index) => {
                            ...
                            <div
                                ...
                                className={`group relative overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-600 ease-[cubic-bezier(0.25,1,0.3,1)] cursor-pointer rounded-[7px] ${
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden bg-theme-text w-full h-full">
                                    <img
                                        src={phase.image}
                                        alt={phase.title}
                                        className="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 ease-[cubic-bezier(0.25,1,0.3,1)] group-hover:scale-110"
                                    />
                                    {/* Dark overlay for text readability */}
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-60 group-hover:opacity-40 transition-opacity duration-600 ease-[cubic-bezier(0.25,1,0.3,1)]" />
                                ...
                                {/* Hover accent line */}
                                <div className="absolute bottom-0 left-0 right-0 h-1 bg-theme-accent transform scale-x-0 group-hover:scale-x-100 transition-transform duration-600 ease-[cubic-bezier(0.25,1,0.3,1)] origin-left" />
                            </div>
                            );
                        })}
                    </div>
